...
if (Get-Command winget -ErrorAction SilentlyContinue) {
    ...
    try {
        winget install Your Name.AIAutoCommit
        if ($LASTEXITCODE -eq 0) {
            $installed = $true
        }
    }
...
if ($installed) {
    ...
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host ""
    Write-Host "To get started, run the interactive setup wizard:" -ForegroundColor Yellow
    Write-Host "  autocommit init" -ForegroundColor Yellow
    Write-Host "" -ForegroundColor Yellow
    Write-Host "This will guide you through:" -ForegroundColor Yellow
    Write-Host "  • Configuring AI provider API keys" -ForegroundColor Yellow
    Write-Host "  • Setting your default model" -ForegroundColor Yellow
    Write-Host "  • Configuring token budget" -ForegroundColor Yellow
    Write-Host "" -ForegroundColor Yellow
    Write-Host "For more information:" -ForegroundColor Yellow
    Write-Host "  autocommit --help" -ForegroundColor Yellow
    Write-Host ""
